...
- **Reset gains cycle semantics:** teardown as today (rewind code to the baseline tag, close open PRs with branch deletion, delete fix branches), then retire the previous cycle's issues (reopen if closed-as-completed, close with reason "not planned"), then file the three fresh issues. Issues already closed as not-planned are left untouched — retirement happens to an issue exactly once, so dead timelines never churn and reset's work does not grow with the number of past cycles. Reset remains the single "make the world fresh" command.
...
- **Setup discovers, runs, merges, labels:** setup locates bug 1's open issue by exact title match (title read from the same answer-key file), runs the runner on it, merges the PR (which closes the issue as `completed`), and applies the `autofixed` label to the issue. Setup first ensures the label exists in the repo (idempotent create-if-absent) — applying an undefined label fails, and no stubbed test can catch a missing repo-level label; only the ritual would. Assisted-lane flows never touch the label.
